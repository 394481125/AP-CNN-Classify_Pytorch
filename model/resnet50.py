...
class ResNet(nn.Module):
    ...
    def pth_nms(self, bboxes, scores, threshold=0.5):
        x1 = bboxes[:, 0]
        y1 = bboxes[:, 1]
        x2 = bboxes[:, 2]
        y2 = bboxes[:, 3]
    ...
    def get_att_roi(self, att_mask, feature_stride, anchor_size, img_h, img_w, iou_thred=0.2, topk=1):
        ...
        with torch.no_grad():
            ...
            for i in range(n):
                ...
                boxes = boxes[score_thred_index, :].cuda()
                scores = scores[score_thred_index].cuda()
                # nms
                nms_index = self.pth_nms(boxes, scores, iou_thred)[:topk]
